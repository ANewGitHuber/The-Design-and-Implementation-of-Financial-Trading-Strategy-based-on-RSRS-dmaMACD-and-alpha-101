...
getOrders <- function(store, newRowList, currentPos, info, params) {
  ...
  if(store$iter > params$rsrs_lookback_m + params$rsrs_lookback){
    ...
    for (i in 1:length(params$series)){
      ...
      #Clear yesterday's position 
      rsrsPos <- -store$rsrsPos
      a006Pos <- -store$a006Pos
      ...
      ###################################################################
      #Adding today's position to the position list
      currentPosition <- append(currentPosition,
                                currentPos[params$series[i]])
      
      #Adding today's date to the date list
      date <<- append(date,index(newRowList[[1]]))
      
      #Adding the position list to the strategy matrix
  ...
  if (store$iter > params$dmalookbacks$long && store$iter > params$macdlookback) {
    ...
    for (i in 1:length(params$series)) {
      ...
      if (last((SMA(store$cl[startIndexma:(store$iter-1),i],n=params$dmalookbacks$short)))
        ...
        #If alpha018 ranking indicates that there is a top 20% ranking on positive price trend, add 100% more position
        if(rank018 <= 0.2){
          dmaPos[params$series[i]] <-  dmaPos[params$series[i]] + abs(short_ma - long_ma)*multiplier_d/long_ma*(posnorm1[i])*100
        }
        
        #If alpha018 ranking indicates that there is a 20%-50% ranking on positive price trend, add 50% more position
        else if(rank018 <= 0.5 && rank018 > 0.2 ){
          dmaPos[params$series[i]] <-  dmaPos[params$series[i]] + (abs(short_ma - long_ma)/long_ma)*100*posnorm1[i]*(multiplier_d/2)
        }
        
        #If alpha018 ranking indicates that there is a under 50% ranking on positive price trend, add 25% more position
        else{
          dmaPos[params$series[i]] <-  dmaPos[params$series[i]] + (abs(short_ma - long_ma)/long_ma)*100*posnorm1[i]*(multiplier_d/4)
        }
        
        #Upward trending of price will stop rapidly
        #Using market orders to liquidate stop loss positions
      ...
      #It is a clear exit point and it also implies the future down-ward trending
        ...
        dmaPos[params$series[i]] <- dmaPos[params$series[i]] - abs(short_ma - long_ma)/long_ma*(posnorm1[i])*100*multiplier_d
  ...
  marketOrders <- marketOrders + a006Pos + dmaPos + rsrsPos
  
  ####################################################################################
  #Conduct position visualization diagram when there's two days left
  if(store$iter==runningDays-2){
    
    #Get the matrix that stores all positions
    strategyMatrix <- strategyMatrix[-1,]
    
    #Loop through all the series
    for(i in 1:length(params$series)){
      
      #Draw position visualization diagram
...
updateDmaPos <- function(store, dmaPos) {
  store$dmaPos <- dmaPos+store$dmaPos
  return(store)
}
