  ...
  if(store$iter > params$rsrs_lookback_m + params$rsrs_lookback){
    ...
    for (i in 1:length(params$series)){
      ...
      if (rsrs_z < 0.7 && alpha006*100 < thr006){
        ...
        # }
        # else if(rank018 <= 0.6 && rank018 > 0.3 ){
        #   rsrsPos[params$series[i]] <- rsrsPos[params$series[i]]*0.2
        # }
        ...
        if (last((SMA(store$cl[startIndexma:store$iter-1,i],n=params$dmalookbacks$short)))
          ...
          dmaPos[params$series[i]] <- -posnorm[i]
        ...
        # if(rank018 <= 0.3){
        # 
        #   rsrsPos[params$series[i]] <- rsrsPos[params$series[i]]*3
        # }
        # else if(rank018 <= 0.6 && rank018 > 0.3 ){
        #   rsrsPos[params$series[i]] <- rsrsPos[params$series[i]]*1.5
        # 
        # }
        # else{
